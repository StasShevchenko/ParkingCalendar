...
class UsersListPageBloc extends Bloc<UsersListPageEvent, UsersListPageState> {
        ...
        switch (event) {
          ...
          case FilterToggled toggleEvent:
            ...
            if (currentRoles.contains(toggleEvent.role)) {
              currentRoles
                  .removeWhere((element) => element == toggleEvent.role);
            } else {
  ...
  void _init() async {
    try {
      ...
      emit(state.copyWith(
          isLoading: false, users: usersList, isConnectionError: false));
    } on DioException catch (exception) {
      if (exception.type == DioExceptionType.connectionTimeout ||
          exception.error is SocketException ||
          exception.type == DioExceptionType.connectionError) {
